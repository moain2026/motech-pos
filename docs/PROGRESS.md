...
### 2026-07-06 — ✨ موجة التميّز (Fable 5): تدقيق نقدي شامل + 8 تحسينات أمان/أداء/UX مُثبتة حياً
تدقيق كامل كمهندس أول (OWASP/أداء/اتساق tokens/a11y) في **`docs/EXCELLENCE_AUDIT.md`** — كل ملاحظة بدليل ملف:سطر أو قياس حي، ثم تنفيذ أعلى التحسينات أثراً:

- **🔒 S1 — كبح brute force على الدخول:** `LoginThrottleService` (5 محاولات فاشلة/15 دقيقة لكل username+IP → RFC 9457 **429 `too-many-login-attempts`** + هيدر `Retry-After`؛ النجاح يصفّر العداد؛ `trust proxy loopback` لالتقاط IP الحقيقي خلف Caddy). **proof حي:** كان `401×6` بلا حد → صار `401×5` ثم `429 / Retry-After: 900`، ودخول admin الصحيح عبر الدومين 200 ✅. +3 اختبارات وحدة.
- **🔒 S2 — إقفال Swagger في الإنتاج:** كان `/api/v1/docs` **مكشوفاً علناً** عبر الدومين (خريطة API كاملة للمهاجم) → يُبنى الآن فقط خارج production (أو بـ`SWAGGER_ENABLED=true`). **proof:** docs/docs-json → 404 محلياً وعبر الدومين ✅.
- **⚡ P1 — تقليص المسار الحرج للبندل:** qrcode+jsbarcode كانتا eager داخل vendor (تُسحبان عبر barrel الـprint من useScannerToCart) → dynamic `import()` + استيراد مباشر بدل barrels + carve-out في manualChunks. **قياس:** vendor 113.1→**95.5KB gz** (بلا JsBarcode/QRCode — فحص binary 0 مطابقة)، qr-lib صار lazy chunk (8.9KB يُحمّل عند الطباعة فقط)، jsbarcode خرج من الـeager graph كلياً؛ إجمالي أول تحميل ~273.7→**258.5KB gz**.
- **💎 C2 — `ConfirmDialog` موحّد فاخر** (promise-based `confirmDialog()` + host في Providers): استبدل **كل الـ12** `window.confirm` الأصلية (إقفال وردية، اعتماد/إلغاء مستندات مخزنية، أوامر بيع، جرد، حذف موازين/اختصارات/برامج ولاء) بحوار داكن RTL على الـtokens بأزرار لمسية وvariant خطر أحمر. لقطة proof: `pos-audit-shots/excellence-confirm-dialog.png`.
- **♿ U1 — focus trap في Dialog المشترك:** تركيز أولي (يحترم autofocus)، حبس Tab/Shift+Tab، إرجاع التركيز للمُطلِق عند الإغلاق، `aria-labelledby` — يستفيد منه 30+ حواراً تلقائياً.
- **♿ U2 — aria-live لتغذية المسح في POS:** live region دائمة `role=status aria-live=polite` — قارئ الشاشة يسمع «أُضيف/غير موجود» لكل مسح باركود.
- **✨ U3 — Skeleton loaders لشبكة الأصناف:** بطاقات هيكلية نابضة (تحترم prefers-reduced-motion) بدل spinner فارغ — لقطة proof حية بإبطاء الشبكة: `excellence-skeleton.png`.
- **🎨 C1 — آخر hex hardcoded** في CustomerDisplayPage → `var(--color-surface)` (اتساق tokens 100% في features).
- **المؤجَّل بأسبابه (في الملف):** إخفاء schemas من /health (قيمة هامشية حالياً) · JWT→httpOnly cookies + CSP (موجة تصلّب معمارية مستقلة) · نشر ذري/CI.
- **الحالة:** tsc -b صفر أخطاء (الاثنان) ✅ · **340 unit** (+3) ✅ · build+deploy → الدومين 200 بباندل `index-DUbHI6yv.js` ✅ · **E2E حي 10/10** (حُدّث اختبار الإقفال للـConfirmDialog) ✅ · pm2 online + /health ok ✅ · 3 commits conventional (9b12b6d أمان، 604948a بندل، b683c57 UX).

### 2026-07-06 — 🔐 موجة التصلّب الأمني (Fable 5): CSP للواجهة + JWT إلى httpOnly cookies (dual-mode) — كل شيء مُثبت حياً
تنفيذ البندين المؤجّلين من EXCELLENCE_AUDIT (S4 + CSP) بدون كسر أي مسار قائم — التفاصيل الكاملة والقياسات في ذيل `docs/EXCELLENCE_AUDIT.md`:

- **🛡️ CSP + رؤوس أمان للواجهة الساكنة:** استجابات Caddy الساكنة كانت **بلا أي رأس أمان** (الـAPI فقط كان خلف helmet). أُضيفت عبر `custom.caddy`: CSP صارمة (`script-src 'self'` — لا inline scripts في dist أصلاً؛ `style-src 'unsafe-inline'` للأنماط فقط لأن React style attributes وطباعة الإيصال تحتاجها؛ `img-src data: blob:` لـQR/الباركود) + nosniff + X-Frame SAMEORIGIN + Referrer-Policy + HSTS + Permissions-Policy (كاميرا/مايك/GPS/دفع/USB مقفلة) + X-Robots-Tag. **proof حي (CDP على الدومين):** الصفحة كاملة تحت الـCSP — SW `activated`، خط Cairo محمّل، **0 أخطاء console و0 انتهاكات CSP**، و`curl -I` يعرض الرؤوس على `/` و`/assets/*`.
- **🍪 S4 — التوكنات إلى httpOnly cookies (تراجُعي):** backend يضبط `mp_at` (access, Path=/) + `mp_rt` (refresh, **Path=/api/v1/auth فقط**) — `HttpOnly; Secure; SameSite=Strict`؛ `JwtAuthGuard` يقبل Bearer (أولوية) أو الكوكي مع **دفاع CSRF مزدوج** (SameSite=Strict + فحص Sec-Fetch-Site على الكتابات)؛ endpoint جديد `POST /auth/logout` يمسح الكوكيز؛ refresh يقبل body أو كوكي. **Frontend:** localStorage يحفظ **profile فقط — صفر JWT** (zustand partialize)، التوكنات in-memory، الجلسة تنجو من reload عبر الكوكي وحده، وانتهاء access يُصلّح نفسه برفرش صامت عبر كوكي الرفرش، والخروج يمسح الكوكيز server-side.
- **proof حي:** curl: login→Set-Cookie(HttpOnly/Secure/Strict) · محمي بالكوكي فقط 200 · refresh بالكوكي 200 · logout→401 بعده · Bearer القديم 200 (توافق) · CSRF cross-site→401 ✅ · **CDP عبر الدومين:** login بالفورم → localStorage بلا أي JWT → document.cookie لا يرى mp_at → reload والجلسة حية → حذف mp_at (بقاء mp_rt) → استعادة صامتة → خروج فِعلي ✅.
- **E2E:** حُدّثت استدعاءات fetch في الاختبارات من قراءة توكن localStorage إلى كوكي المتصفح — **10/10 تمر على النظام الحي بعد النشر** (19.7s).
- **الحالة:** tsc -b صفر (الاثنان) · oxlint 0 errors · **348 unit** (+8 لـauth-cookies/guard) · build+deploy → الدومين 200 بباندل `index-BEPzU9X9.js` · pm2 online /health ok · نسخة Caddy الاحتياطية في `/var/backups/custom.caddy.bak-security-wave`.
