...
| POST022 | جرد أصناف مردود المبيعات | مطابقة الكمية المردودة فعلياً مع المسجلة | 🟡 | ✅ `POST/GET /return-counts` + `/:id/lines` + `/:id/post` (V026: النظامي حي من IAS_POS_RT_BILL_MST/DTL لآلة+يوم، DIFF=معدود−نظامي، اعتماد supervisor idempotent) (2026-07-04 Fable5) | ❌ | P2 | تبقى شاشة الواجهة |
...
| POST024 | طلبات العملاء | أوامر بيع تُنزَّل لاحقاً في فاتورة | 🟡 | ✅ module sales-orders: `POST/GET /sales-orders` + `/:id/convert` (**ينزل فاتورة حقيقية في YSPOS23** عبر PostBillUseCase — مُثبت: أمر→فاتورة 260700300000407) + `/:id/cancel` (V021: OPEN→CONVERTED|CANCELLED، تحويل idempotent) (2026-07-04 Fable5) | ❌ | P2 | تبقى شاشة الواجهة |
...
| POST028 | التحويل المخزني | تحويل أصناف بين المخازن (36 block) | 🟡 | ✅ `POST/GET /stock-issues` + `/:id/post` + `/:id/cancel` (V025: اعتماد supervisor يكتب **ITEM_MOVEMENT حقيقي DOC_TYPE=7/IN_OUT=−1** + حارس توفر 422 insufficient-stock — مُثبت: المخزون نقص فعلياً 35→32) (2026-07-04 Fable5) | ❌ | P2 | تبقى شاشة الواجهة |
| POST029 | الاستلام المخزني | استلام التحويلات الواردة | 🟡 | ✅ `POST/GET /stock-receipts` + `/:id/post` + `/:id/cancel` (V022+V023: اعتماد supervisor يكتب **ITEM_MOVEMENT حقيقي DOC_TYPE=8/IN_OUT=+1 فيزيد المخزون فعلاً** — مُثبت: 33→35 وصف SERIAL=13929 بSELECT مباشر؛ V024 أصلح 158 حزمة ERP كانت تكسر triggers) (2026-07-04 Fable5) | ❌ | P2 | تبقى شاشة الواجهة |
...
**POST: ✅ 14 · 🟡 14 · ❌ 2 (كلاهما N/A: POSTIN_MTX وPOST_WST)** _(2026-07-04 Fable5 backend: POST022/024/028/029 صارت 🟡 — backend كامل مُثبت حياً، تنقصها الواجهة فقط)_ _(تدقيق فجر 2026-07-04: +POST019 وPOST023 بواجهتيهما — POST018 يبقى 🟡 بلا UI)_ _(2026-07-04: POST001/010/013/021 صارت ✅ — الواجهة رُبطت بالكامل)_
...
**POSS: ✅ 2 · 🟡 2 · ❌ 5** _(تدقيق فجر 2026-07-04: POSS001 وPOSS004 ✅ مُثبتتان حياً)_
...
| POS_ALRT_SCR | خدمي | تنبيهات/ملاحظات عند الدخول | 🟡 | ✅ `POST/PUT/GET /alerts` + `GET /alerts/pending` (غير المُقرّة للمستخدم الحالي) + `POST /:id/ack` (مرة لكل مستخدم، V026) (2026-07-04 Fable5) | ❌ | P2 | popup عند الدخول في الواجهة |
...
**أخرى: ✅ 3 · 🟡 3 (POSADVS، POS_ITM_PRICE، POS_ALRT_SCR) · ❌ 4**
...
|---|:---:|:---:|:---:|:---:|
| حركات POST | 30 | **14** | 14 | 2 |
| تقارير POSR | 16 | **15** | 1 | 0 |
| إدخال POSI | 15 | **7** | 4 | 4 |
| إعدادات POSS | 9 | **2** | 2 | 5 |
| أخرى | 10 | **3** | 3 | 4 |
| **الإجمالي** | **80** | **41** | **24** | **15** |

> تفصيل العدّ الصارم (لمراجعة مستقبلية):
> - **POST ✅ 14:** 001,002,003,004,010,013,016,017,019(TransfersPage),021,023(PrescriptionsPage),025,026,027 · 🟡 14: 005,006,008,009,011,012,014,015,018(بلا UI),020,**022,024,028,029 (backend كامل 2026-07-04 Fable5 — بلا UI)** · ❌ 2: POSTIN_MTX,POST_WST (كلاهما N/A)
> - **POSR ✅ 15:** الكل عدا POSR011 🟡 (backend returns-window ✓ — تبويب واجهة مفقود). POSR003 ✅ بزر CSV الحي (مُثبت curl: `reports/export?report=daily` يرجع CSV فعلياً)
> - **POSI ✅ 7:** 001,007,009,010,011,200,2000 · 🟡 4: 002,003,008,012 · ❌ 4: 004,005,006,014(N/A)
> - **POSS ✅ 2:** 001 (179 إعداداً مُثبتة بالمتصفح حياً: 10 تبويبات × مجموع 179)، 004 (change-password — مُثبت 401 لكلمة قديمة خاطئة) · 🟡 2: 002 (UI موجود — الفرض الديناميكي ناقص)، 005 (بلا تبويب defaults) · ❌ 5: 003 + 4×WMS(N/A)
> - **أخرى ✅ 3:** POSLGN,POSQ001,POSAVLQTY · 🟡 2: POSADVS,POS_ITM_PRICE · ❌ 5
...
- **ضمن النطاق التشغيلي الفعلي (P0+P1 = 44 شاشة):** ✅ 29 · 🟡 14 · ❌ 1 (شاشة العميل الثانية فقط) ⇒ **81.8% مرجّحة** وكل الـP0 الثماني ✅ 100%.
