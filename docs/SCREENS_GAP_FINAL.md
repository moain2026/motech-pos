...
| POST010 | الدفع النقدي للفواتير | تدفيع الفواتير النقدية (كاشير تدفيع منفصل) | 🟡 | ✅ `POST /bills/{id}/payments(/multi)` | 🟡 PaymentDialog داخل سياق البيع فقط | P1 | شاشة تحصيل مستقلة: بحث فاتورة غير مدفوعة → دفع (وضع "كاشير تدفيع") |
| POST011 | سداد الفواتير | سداد فواتير آجلة (IAS_POS_PAY_BILLS) | 🟡 | ✅ نفس payments endpoints | ❌ لا UI سداد من BillDetail | P1 | زر "سداد" في BillDetailPage للفواتير CREDIT غير المسددة + سجل السدادات |
...
| POST013 | تصفية مبيعات الكاشيرات | عدّ النقد بالفئات ومطابقته مع النظام | 🟡 | ✅ `GET /shifts/{id}/reconciliation` | ✅ ReconciliationPage (X-report حيّ) | **P0** | **ناقص:** إدخال العدّ بفئات العملة (denominations) + **حفظ** سجل تصفية معتمد (POST) بدل العرض المحسوب فقط |
...
| POST021 | استعراض حركة النقاط | سجل حركة النقاط لكل العملاء + الانتهاء | 🟡 | 🟡 `loyalty/customers/{code}/points` (رصيد فقط) | 🟡 الرصيد في CustomersPage | P1 | endpoint سجل حركة النقاط (كسب/استبدال/انتهاء) + تبويب حركة في صفحة العميل |
...
|---|---|---|:---:|---|---|:---:|---|
| POSI001 | أجهزة وآلات نقاط البيع | تعريف الآلات + التصريح للأجهزة | 🟡 | 🟡 `GET /admin/machines` (قراءة) | 🟡 AdminPage (قراءة) | P1 | CRUD آلة (`POST/PUT /admin/machines`) + تصريح جهاز (ربط terminal بالآلة) |
...
| POSI011 | المستخدمون | إدارة مستخدمي النظام (USER_R) | 🟡 | 🟡 `GET /admin/users` (قراءة) | 🟡 AdminPage (قراءة) | P1 | CRUD مستخدم (إنشاء/تعطيل/دور/آلة افتراضية) |
...
| POSI2000 | بيانات الأصناف | إدخال/تعديل الأصناف (IAS_ITM_MST) | 🟡 | ✅ `POST/PUT /items` | ✅ ItemsPage + ItemDialog | P1 | **ناقص:** وحدات متعددة، باركودات متعددة للصنف، مستويات أسعار — الحالي (كود/اسم/باركود واحد/وحدة/سعر واحد) |
...
| POSS002 | صلاحيات المستخدمين | صلاحيات تفصيلية للشاشات/العمليات | 🟡 | 🟡 RBAC ثابت (roles: cashier/supervisor/admin) | 🟡 RequireRole في الراوتر | P1 | صلاحيات دقيقة قابلة للإدارة (خصم أقصى، إلغاء سطر، فتح درج…) + شاشة إدارة |
...
| POS_ITM_PRICE | خدمي | مستويات أسعار الأصناف (DFLT_PRICE_LEV) | 🟡 | 🟡 سعر واحد يُقرأ ضمن الصنف | ❌ | P1 | دعم مستويات الأسعار (lev_no) في catalog + اختيار المستوى بالآلة/العميل |
...
| 2 | POST013 🟡 | إدخال العدّ بفئات العملة + **حفظ** سجل تصفية معتمد (الحالي عرض محسوب فقط) |
...
|---|---|:---:|---|
| 5 | POST010/011 | 🟡 | شاشة تحصيل/سداد فواتير مستقلة (backend جاهز) |
...
| 10 | POST021 | 🟡 | سجل حركة النقاط التفصيلي |
...
| 16 | POSI011 | 🟡 | CRUD مستخدمين (الحالي قراءة) |
| 17 | POSI001 | 🟡 | CRUD آلات + تصريح أجهزة (الحالي قراءة) |
| 18 | POSI2000 | 🟡 | وحدات/باركودات متعددة + مستويات أسعار للصنف |
| 19 | POSI002+003 | ❌ | تكوين لوحات المفاتيح اللمسية وربط ItemGrid بها |
| 20 | POSS004 | ❌ | تغيير كلمة السر (جهد ساعة) |
| 21 | POSS002 | 🟡 | صلاحيات تفصيلية قابلة للإدارة |
...
| 25 | POS_ITM_PRICE | 🟡 | مستويات الأسعار (يتكامل مع #18) |
...
2. **تصفية معتمدة (POST013/015):** `POST /shifts/{id}/reconcile` بإدخال فئات العملة + حفظ سجل over/short.
