# POSR005 — توثيق شاشة Onyx Pro POS

> النوع: **POSR** — تقارير (Reports)  
> الحجم: 324,004 bytes · فك witchi: ✅ كامل · المصدر: `D:\YS_ERP\Forms\POSR005.fmx`

## 1. الوظيفة

_(لا يوجد ملف مساعدة مطابق؛ الوظيفة مُستنتَجة من النوع والـ SQL)_

شاشة من فئة **تقارير (Reports)**. أكثر الجداول استخداماً: `USER_R`, `S_RPRT_USR_TMPLT_MST`, `FORM_DETAIL`, `S_RPRT_USR_TMPLT_DTL`, `USER`, `IAS_POS_BILL_MST`.
...
**قياسية (Forms events):** `KEY-COMMIT`, `KEY-CREREC`, `KEY-DELREC`, `KEY-DOWN`, `KEY-ENTER`, `KEY-ENTQRY`, `KEY-EXEQRY`, `KEY-EXIT`, `KEY-LISTVAL`, `KEY-MENU`, `KEY-NEXT-ITEM`, `KEY-OTHERS`, `KEY-PREV-ITEM`, `KEY-PRINT`, `KEY-SCRDOWN`, `KEY-SCRUP`, `KEY-UP`, `ON-ERROR`, `ON-LOCK`, `ON-MESSAGE`, `PRE-FORM`, `WHEN-NEW-FORM-INSTANCE`, `WHEN-TAB-PAGE-CHANGED`, `WHEN-TIMER-EXPIRED`, `WHEN-WINDOW-ACTIVATED`, `WHEN-WINDOW-CLOSED`, `WHEN-WINDOW-RESIZED`
...
**Procedures/Functions (9):** `CLEAR_PROC`, `EXIT_PROC`, `FILL_ALL_LIST_PRC`, `LOAD_PARAMETERS`, `PRE_FORM_PRC`, `PRINT_PROC`, `WHEN_NEW_FORM_INSTANCE_PRC`, `SHW_PRNT_DATA_PRC`, `AUD_PRNT_REP_PRC`
...
**حِزَم DB المُستدعاة (PL/SQL packages):** `IAS_GEN_PKG`, `GEN_PKG`, `IAS_CHECK_SYS_PKG`, `LOV_PKG`, `IAS_PRMTR_PKG`, `IAS_USR_PKG`, `GEN_RPRT_PKG`, `SETUP_PKG`, `YS_GEN_PKG`, `IAS_BRN_PKG`, `IAS_FETCH_DATA_PKG`

## 10. عيّنة جُمل SQL مُستخرَجة (proof)
```sql
SELECT AUDIT_DEL_ITM FROM IAS_PARA_POS
Select bill_no From ias_pos_bill_mst where HUNG=0 and bill_no=Ias_Pos_History_Hung_Bills.bill_no union Select bill_no From IAS_POS_HST_BILL_MST where HUNG
...
select 1 from privilege_gc where u_id=:Parameter.User_no and g_code=ias_itm_mst.g_code and add_flag=1) order by i_code I_CODE I_NAME I_DESC RG_FORM_NO select IAS_FORM_NAME.form_no, IAS_FORM_NAME.form_name, F_FILE_NAME FROM FORM_DETAIL,IAS_FORM_NAME
SELECT BLK_NM,ITM_NM,VAL FROM S_RPRT_USR_TMPLT_DTL WHERE TMPLT_NO
SELECT 1 FROM S_RPRT_USR_TMPLT_MST WHERE GNR_FOR_ALL_USR_FLG = 1 ) )ORDER BY SQ_NO" MS Sans Serif SearchTree1 From machine
```
...
- عدد الجُمل/الأسطر في ملف strings: راجع `_raw/POSR005_strings.txt`.
