# POSR013 — توثيق شاشة Onyx Pro POS

> النوع: **POSR** — تقارير (Reports)  
> الحجم: 344,200 bytes · فك witchi: ✅ كامل · المصدر: `D:\YS_ERP\Forms\POSR013.fmx`

## 1. الوظيفة

_(لا يوجد ملف مساعدة مطابق؛ الوظيفة مُستنتَجة من النوع والـ SQL)_

شاشة من فئة **تقارير (Reports)**. أكثر الجداول استخداماً: `IAS_ITM_MST`, `S_RPRT_USR_TMPLT_MST`, `USER_R`, `PRIVILEGE_GC`, `S_RPRT_USR_TMPLT_DTL`, `IAS_CASH_CUSTMR_GRP`.
...
SELECT USE_ADVNC_PYMNT_TYP FROM IAS_PARA_POS
select i_code from Ias_Itm_Mst where Assistant_No = and order_detail.i_code in (select i_code from Ias_Itm_Mst where Assistant_No between
select i_code from Ias_Itm_Mst where Detail_No = and order_detail.i_code in (select i_code from Ias_Itm_Mst where Detail_No between
...
- عدد الجُمل/الأسطر في ملف strings: راجع `_raw/POSR013_strings.txt`.
